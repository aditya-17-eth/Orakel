...
export function humanizeContractError(error: unknown) {
  ...
  if (lower.includes("loansdisabled") || lower.includes("#29")) return "loans are not enabled for this contract yet";
  if (lower.includes("loanalreadyexists") || lower.includes("#30")) return "repay your current loan before opening another one";
  if (lower.includes("loantoolarge") || lower.includes("#32")) return "this loan exceeds the 3x leverage limit";
  if (lower.includes("loanreservetoosmall") || lower.includes("#33")) return "the loan pool does not have enough available funds";
