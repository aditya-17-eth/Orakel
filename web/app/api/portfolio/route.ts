  ...
  try {
    ...
    for (let start = 0; start < count; start += batchSize) {
      ...
      const batch = await Promise.all(ids.map(async (id) => {
        ...
        return { marketId: id, market, position, lpShares, loan, markValue, claimable };
      }));
      positions.push(...batch.filter((row): row is NonNullable<typeof row> => row !== null));
    }
    const totals = positions.reduce((sum, row) => ({ spent: sum.spent + row.position.spent, markValue: sum.markValue + row.markValue, claimable: sum.claimable + row.claimable, debt: sum.debt + row.loan.debt }), { spent: 0n, markValue: 0n, claimable: 0n, debt: 0n });
